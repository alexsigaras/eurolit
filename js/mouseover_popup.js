...
function show(imagename,title,width,height){
    ...
    var docwidth = window.pageXOffset + window.innerWidth - offsetfrommouse[0]
	...
	if(docwidth > 650 && docheight > 500) {
		...
		newHTML = '<div class="border_preview" style="margin-left:200px;margin-top:-100px;width:'+  width +'px;height:'+ height +'px"><div id="loader_container"><div id="loader"><div align="center">Loading image preview...</div><div id="loader_bg"><div id="progress"> </div></div></div></div>';
...
function followmouse(e){
	...
	if (typeof e != "undefined"){
		...
		if (docheight - e.pageY < defaultimageheight + 2*offsetfrommouse[1]){
			ycoord += e.pageY - Math.max(0,(2*offsetfrommouse[1] + defaultimageheight + e.pageY - docheight - window.pageYOffset));
		} else {
			ycoord += e.pageY;
		}
		...
		if (docwidth - event.clientX < defaultimagewidth + 2*offsetfrommouse[0]){
			xcoord = event.clientX + window.pageXOffset - xcoord - defaultimagewidth; // Move to the left side of the cursor
		} else {
			xcoord += window.pageXOffset+event.clientX
		}
		if (docheight - event.clientY < (defaultimageheight + 2*offsetfrommouse[1])){
			ycoord += event.clientY + window.pageYOffset - Math.max(0,(2*offsetfrommouse[1] + defaultimageheight + event.clientY - docheight));
		} else {
			ycoord += window.pageYOffset + event.clientY;
		}
	}
